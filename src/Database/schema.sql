-- Corrected WeeBunz Database Schema
-- Ensures tables are created before being referenced by foreign keys.

-- Tables without external dependencies (except wp_users)
CREATE TABLE IF NOT EXISTS `{prefix}quiz_types` (
...
CREATE TABLE IF NOT EXISTS `{prefix}questions_pool` (
    ...
    `category_id` bigint(20) UNSIGNED, -- Changed from varchar category
    ...
    CONSTRAINT `fk_question_category` FOREIGN KEY (`category_id`) 
        REFERENCES `{prefix}quiz_categories` (`id`) ON DELETE SET NULL
) {charset_collate};
...
-- Tables depending on the first group + wp_users
...
-- Tables depending on the second group
...
-- Tables depending on the third group
...
CREATE TABLE IF NOT EXISTS `{prefix}quiz_sessions` (
    ...
    `quiz_id` bigint(20) UNSIGNED NOT NULL, -- Changed from quiz_type_id
    `user_id` bigint(20) UNSIGNED,
